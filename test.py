...
def dataGenerationViewers(dateList, df, sc_df):
    result = pd.DataFrame(columns=['date', 'channelId', 'chats', 'memberChats', 'uniqueChatters', 'uniqueMembers',
                               'superChats', 'uniqueSuperChatters', 'totalSC', 'totalLength', 'impact1', 'impact2',
                               'impact3', 'impact4', 'impact5', 'impact6', 'impact7'])
    ...
    for date in dateList:
        ...
        uniqueSuperChatters = unique_tmp_sc['authorChannelId'].value_counts().to_dict()
        totalLength = tmp.groupby(['authorChannelId'])['bodyLength'].sum().to_dict()
        totalSC = tmp_sc.groupby(['authorChannelId'])['amount'].sum().to_dict()
        impact1 = tmp_sc[tmp_sc['significance'].isin([1])].groupby(['authorChannelId'])['significance'].count().to_dict()
        impact2 = tmp_sc[tmp_sc['significance'].isin([2])].groupby(['authorChannelId'])['significance'].count().to_dict()
        impact3 = tmp_sc[tmp_sc['significance'].isin([5])].groupby(['authorChannelId'])['significance'].count().to_dict()
        impact4 = tmp_sc[tmp_sc['significance'].isin([10])].groupby(['authorChannelId'])['significance'].count().to_dict()
